...
> Can be useful for Typedown: For designing new URI schemes, see [RFC 2718](https://www.rfc-editor.org/rfc/rfc2718).

![URI structure](../../../../assets/uri.svg)
...
- **Uniform**: A bare-bone, base form for identifiers. Because the syntax is shared:
...
- **Interpretation depends only on the characters, not on how they're represented in a network protocol.** (should be normalized into 1 canonical form)
...
- **userinfo**:
  - **Optional** subcomponent. Some disallow this.
  - The `user:password` format is **deprecated**, although still used by some, like many DBMS connection URI (cleartext credentials).
  - Should render distinctly to prevent phishing (e.g. `http://trusted.com@evil.com/`).
- **host**:
  - **Mandatory** subcomponent.
  - IP literal (`[2001:db8::7]`), IPv4 (`192.0.2.1`), or registered name (`example.com`).
  - **Case-insensitive**.
  - Having a host does **NOT** imply Internet access.
- **port**:
  - **Optional** subcomponent. Some disallows this.
  - Schemes may define **defaults** (e.g. `http` = 80).
  - Should omit if empty or equal to default.
